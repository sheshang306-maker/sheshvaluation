...
# ────────────────────────────────────────────────────────────────────────────
MIN_DELAY_S      = 1.3    # minimum pause between Yahoo requests
MAX_DELAY_S      = 3.0    # maximum pause (random jitter)
MAX_RETRIES      = 6      # retry budget per call
BASE_BACKOFF_S   = 4.0    # base for exponential backoff on 429/401
...
# ────────────────────────────────────────────────────────────────────────────
def _with_retry(fn, *args, on_retry=None, **kwargs):
    """
    ...
    on_retry: optional zero-arg callable invoked before each retry attempt
              (after the backoff sleep). Used to reset stale state — e.g.
              force a fresh session/crumb handshake — before trying again.
    """
    last_exc = None
    for attempt in range(MAX_RETRIES):
        ...
        try:
            ...
            last_exc = exc
            msg = str(exc).lower()
            # Retryable: ordinary rate-limiting AND session/crumb-auth
            # failures. A "401 Invalid Crumb" happens when Yahoo's
            # session-auth token goes stale mid-run — it isn't a
            # permanent error, a fresh session (see on_retry) usually
            # fixes it, so it must not be treated as fatal.
            is_retryable = any(x in msg for x in (
                "429", "rate", "too many", "forbidden", "403",
                "401", "unauthorized", "crumb",
            ))
            if not is_retryable:
                # Non-retryable error — don't keep retrying
                raise
            logger.warning("yf_ratelimit: retryable error on attempt %d: %s", attempt + 1, exc)
...
class _CachedTicker:
    ...
    def _refresh_session(self):
        """
        Force a brand-new session (and Ticker object) on the next _get_yf()
        call. Used as the on_retry hook so a stale/expired crumb — the
        "401 Invalid Crumb" case — doesn't keep failing on every retry with
        the same bad session.
        """
        with self._yf_lock:
            self._yf_obj = None
        # yfinance also keeps its own process-wide crumb/cookie cache
        # (independent of the session object we pass in). Clear it too if
        # this yfinance version exposes it, so the next Ticker() call is
        # forced to re-negotiate a fresh crumb instead of reusing the
        # stale one that triggered the 401.
        try:
            from yfinance.data import YfData
            inst = YfData._instance if hasattr(YfData, '_instance') else None
            if inst is not None:
                for attr in ("_crumb", "crumb", "_cookie", "cookie"):
    ...
    def _fetch_prop(self, prop: str) -> Any:
        ...
        result = _with_retry(_do, on_retry=self._refresh_session)
        _mem_set(key, result)
        return result
    ...
    def history(self, period="1mo", interval="1d", **kwargs) -> pd.DataFrame:
        # yfinance treats `period` and explicit `start`/`end` as mutually
        # exclusive — passing both can raise. If the caller supplied a date
        # range, drop the default period so start/end take effect cleanly.
